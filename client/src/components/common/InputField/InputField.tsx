...
const InputField: React.FC<InputFieldProps> = ({placeholder, type="text", value, onChange, name, disabled=false,})=>{
    ...
    return <>
        <div className={InputFieldStyle["input-wrapper"]}>
            <input type={type==="password"? (hidden? type:"text"):type} name={name} placeholder={placeholder} value={value} onChange={onChange} disabled={disabled} className={InputFieldStyle["input-field"]}/>
